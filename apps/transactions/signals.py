...
# Import Models
from .models import Transaction, TransactionItem, TransactionItemSource
...
def handle_transaction_status_change(sender, instance, created, **kwargs):
    ...
    if old_status != Transaction.StatusChoices.COMPLETED and new_status == Transaction.StatusChoices.COMPLETED:
        ...
        for tx_item in instance.items.all():
            ...
            # Validasi Stok
            if qty_needed > 0:
                # Jika stok di PO kurang, tetap kurangi master inventory tapi log warning (bisa jadi stok awal non-PO)
                pass 
...
def update_transaction_total_item(sender, instance, **kwargs):
    _update_transaction_total(instance.transaction)

@receiver([post_save, post_delete], sender=TransactionItemSource) # Jika butuh trigger lain
def update_dummy(sender, instance, **kwargs):
    pass

def _update_transaction_total(txn):
    """Helper untuk update total transaksi"""
    ...
    new_total = total_items + total_services + total_miscs + txn.other_charges - txn.discount_amount
