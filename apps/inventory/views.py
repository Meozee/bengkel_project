...
def inventory_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save() # -> Ini akan save field (name, sku, dll)
                        # -> quantity & buy_price akan diisi 'default=0' dari models.py
            messages.success(request, "Item berhasil ditambahkan!")
            return redirect('inventory:inventory_list')
    else:
        # Form-nya bersih, tidak perlu 'initial'
        form = InventoryForm() 
...
def inventory_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save() # -> Ini HANYA akan save field yang ada di form
                        # -> quantity & buy_price di database TIDAK AKAN TERSENTUH
...
# ==============================
# Semua view kategori ini sudah benar
